...
def generate_prompt(text):
    ...
    try:
        ...
        prompt_request = f"Create a detailed professional image generation prompt in English for: {text}. Make it long, descriptive, and include details about scene, lighting, colors, composition, and atmosphere."
        ...
        if response.status_code == 200 and response.text.strip():
            ...
            return clean_prompt if clean_prompt else f"Create a high-quality, detailed image of: {text}. Realistic details, vibrant colors, professional lighting, 4K resolution."
        else:
...
def generate_edit_prompt(text):
    ...
    try:
        ...
        prompt_request = f"Create a detailed professional image editing prompt in English for: {text}. Focus on specific modifications like color adjustments, lighting changes, background alterations, enhancements, and technical improvements."
        ...
        if response.status_code == 200 and response.text.strip():
            clean_prompt = clean_prompt_text(response.text)
            return clean_prompt if clean_prompt else f"Professional image editing prompt for: {text}. Enhance quality, adjust colors, improve lighting, refine details."
        else:
            return f"Professional image editing prompt for: {text}. Enhance quality, adjust colors, improve lighting, refine details."
    except Exception as e:
        return f"Professional image editing prompt for: {text}. Enhance quality, adjust colors, improve lighting, refine details."

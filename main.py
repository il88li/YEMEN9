...
def generate_edit_prompt(text):
    ...
    try:
        response = gemini_model.generate_content(
            f"أنت مساعد محترف في إنشاء مطالبات تعديل الصور. أنشئ مطالبة تعديل احترافية باللغة العربية بناءً على الطلب التالي: {text}. يجب أن تكون المطالبة مفصلة وتصف التعديلات المطلوبة بدقة."
        )
        return response.text.strip()
    except Exception as e:
        return f"مطالبة تعديل احترافية لل: {text}. تحسين الجودة، تعديل الألوان، تحسين التفاصيل، إضاءة مثالية."
